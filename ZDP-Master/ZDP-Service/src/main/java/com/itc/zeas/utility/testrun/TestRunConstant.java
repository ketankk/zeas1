...
public class TestRunConstant {
	...
	static {
		...
		MAP_RED_TESTRUN_HOME = System.getProperty("user.home") + "/zeas/";
	...
	private static void initHdfsAndRManagerURL() {
		RManagerUrl = ConfigurationReader.getProperty("JOB_TRACKER");
		HdfsUrl = ConfigurationReader.getProperty("HDFS_FQDN");
		LOGGER.debug("VALUE FROM CONFIG PROPERTY -- RManagerUrl: "
				+ RManagerUrl + "--HdfsUrl: " + HdfsUrl);
		// TODO to be deleted since we should take config properties fom
		// centralized place.
		/*
		 * try { InputStream inputStream = new FileInputStream(
		 * System.getProperty("user.home") + "/zeas/Config/config.properties");
		 * Properties prop = new Properties(); prop.load(inputStream);
		 * RManagerUrl = prop.getProperty("JOB_TRACKER"); HdfsUrl =
		 * prop.getProperty("HDFS_FQDN"); } catch (IOException e) {
		 * logger.error(
		 * "problem in reading content from config.properties file");
		 * e.printStackTrace(); }
		 */
	}
}
