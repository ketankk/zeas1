...
public class XmlToCsv extends DefaultHandler implements Runnable {
	...
	// this is a user defined Exception class used for logically break the
	// reading at 1000th record
	...
	public XmlToCsv(String XmlFileName, boolean isSmall) {
		...
		outLargeFile = ConfigurationReader.getProperty("APP_DIR") + "/"
				+ largeArr[largeArr.length - 1];
	...
	private void writeToCsvBigFile() {
		...
		/*
		 * try { foutLarge = new FileWriter(bigFile); } catch (IOException e2) {
		 * e2.printStackTrace(); }
		 */
		...
		try {
			...
			logger.error("Sax Exception failed to parce -------->"
					+ e.toString());
			if (bigFile.exists()) {
				bigFile.delete();
			}

			logger.info("Deleted the " + outLargeFile
					+ " file due to parse failuer");
	...
	/**
	 * This method will write the csv file after converting xml file to csv
	 * format. It will write full csv file and also sample file having 1000
	 * records. uses multithreading for parallel writing of csv file and get the
	 * datatype from file uses map containing key as tagname and value as a list
	 * of element present in tag
	 * 
	 * @throws IOException
	 */
	private void writeToCsv() throws IOException {
		...
		if (writeCount < 1) {
			...
			foutsmall.close();
			logger.info("Sample data written to sample file as csv --------"
					+ outSmallFile);
			writeCount++;
		}
	...
	private void cleanData() {
		...
		if (tempKey.size() > 1) {
			...
			xmlEndTag = tempKey.get(tempKey.size() - 1);
		System.out.println(xmlEndTag);
	...
	public void characters(char[] ac, int i, int j) throws SAXException {
		String tmpValues = new String(ac, i, j).trim().replaceAll(",", "")
				.replaceAll("\n", "");
