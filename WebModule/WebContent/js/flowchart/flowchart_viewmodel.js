...
(function () {
	...
	// Width of a node.
	//
	flowchart.nodeWidth = 156;

	//
	// Amount of space reserved for displaying the node's name.
	//
	flowchart.nodeNameHeight = 25;
	...
	flowchart.ChartViewModel = function (chartDataModel) {
		//alert('hi');
//console.log("flowchart.ChartViewModel = TRUE");
		//
	...
	//	console.log(this.connections)
		...
		this.createNewConnection = function (sourceConnector, destConnector) {
			...
			var connectionDataModel = {
				source: {
					...
					desnodeID: destNode.data.id,
					desnodeName: destNode.data.name,
					sourceNodeSchema: sourceNode.data.nodeSchema,
					destNodeSchema: destNode.data.nodeSchema,
					connectorIndex: sourceConnectorIndex,
					inputConnect: previnputConnect,
				},
				dest: {
			...
			if(sourceNode.inputConnectors[0] != undefined){
				previnputConnect = sourceNode.inputConnectors[0].data.name;
			}
			else{
				previnputConnect = '';
			}
		...
		this.deleteSelected = function ($scope) {
			...
			//$scope.deleteStage(node.data.id);
