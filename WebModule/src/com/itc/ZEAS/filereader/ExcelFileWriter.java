...
import com.itc.taphius.utility.ConfigurationReader;
	...
	/**
	 * This method will write the csv file after converting Excel sheet to csv format.
	 * It will write two different csv file one for sample data i.e. 1000 lines another complite file.
	 * uses multithreading for parallel writing of csv file.
	 * @throws IOException
	...
	private void csvWriter() throws IOException, OpenXML4JException,
		...
		if (FileName.endsWith("xlsx")) {
			...
			while(iter.hasNext()){
				...
				outSmallFile = FileName.replace(".xlsx","_"+count+ FileReaderConstant.SAMPLE_FILE);
			...
			if (limitCount) {
				...
				logger.info("Preparing to Write sample CSV file from XLSX file as Output File Name "+ outSmallFile);
				XlsxToCsv xlsx2csv = new XlsxToCsv(p, pr,stream, minColumns, true);
				xlsx2csv.process();
				logger.info(" Output sample File Written Successfuly as  "+ outLargeFile);
			} else {
				pr = new PrintStream(outLargeFile);
				logger.info("Preparing to Write the CSV file from XLSX file as Output File Name "+ outLargeFile);
				XlsxToCsv xlsx2csv = new XlsxToCsv(p, pr,stream, minColumns, false);
				xlsx2csv.process();
				logger.info(" Output File Written Successfuly as  "+ outLargeFile);
			}
			++count;
			}
			...
			NPOIFSFileSystem npoifsFileSystem = new NPOIFSFileSystem(new File(FileName));
			HSSFWorkbook workbookh = new HSSFWorkbook(npoifsFileSystem.getRoot(), true);
			int numSheet = workbookh.getNumberOfSheets();
			for(int m = 0;m<numSheet;m++){
				outSmallFile = FileName.replace(".xls","_"+(m+1)+ FileReaderConstant.SAMPLE_FILE);
				...
				String[] largeArr=outLargeFile.split("\\|");
				outLargeFile=ConfigurationReader.getProperty("APP_DIR")+"/"+largeArr[largeArr.length-1];
				System.out.println("local path small xls :**************:"+outSmallFile);
				System.out.println("local path large xls:**************:"+outLargeFile);
			...
			if (limitCount) {
				...
				logger.info("Total No of Rows for complite file-->>>>" + numRow);
			}
					...
					if (column != null) {
						...
						case Cell.CELL_TYPE_NUMERIC:
							...
							if (DateUtil.isCellDateFormatted(column)) {
								...
								ColumValueList.append(df
										.formatCellValue(column));
							} else

								thisStr = formatter.formatRawCellContents(
										column.getNumericCellValue(),
										column.getCellStyle().getDataFormat(),
										column.getCellStyle()
												.getDataFormatString());
						...
						case Cell.CELL_TYPE_BOOLEAN:
							...
							ColumValueList.append(column.getBooleanCellValue());
						...
						case Cell.CELL_TYPE_FORMULA:
							...
							if (workbookh.getCreationHelper()
									.createFormulaEvaluator().evaluate(column)
		...
		if (limitCount) {
			logger.info("Excel to sample CSV file converted successfuly Output stored as   ------>"+outSmallFile);
		} else
			logger.info("Excel to complite CSV file converted successfuly Output stored as ------>"+outLargeFile);					
	}
}
